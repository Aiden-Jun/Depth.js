...
function idxConvert(idx120) {
    const rank = Math.floor(idx120 / 10) - 2;
    const file = (idx120 % 10) - 2;
    return rank * 8 + file;
}
...
class BoardState {
    constructor(boardArray, currentColor, castlingRights, enPasT) {
...
class Logics {
    ...
    kingMoves(color, board, index) {
        ...
        [-10, -9, -1, 1, 9, 10, 11, -11].forEach(off => {
            const to = index + off;
            if (!ob.includes(to) && (A[to] === pieces.e || gpc(A[to]) === goc(color)) && !this.isIndexAttacked(to, board, goc(color))) {
                moves.push(new Move(index, to, null, A[to] !== pieces.e));
            }
        });
    ...
    minimax(board, depth, alpha, beta, maximizingPlayer) {
        ...
        if (maximizingPlayer) {
            ...
            for (const move of legalMoves) {
                ...
                if (beta <= alpha) {
                    if (this.showThought) {
                        console.log(`[Depth ${depth}] Beta cutoff at move ${move.getUCI()}`);
                    }
                    break;
                }
            }
            ...
            for (const move of legalMoves) {
                ...
                if (beta <= alpha) {
                    if (this.showThought) {
                        console.log(`[Depth ${depth}] Alpha cutoff at move ${move.getUCI()}`);
                    }
                    break;
                }
            }
